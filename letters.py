...
class LetterCycleGame:
    def __init__(self, root):
        self.root = root
        self.letters = ['A', 'B', 'C', 'D', 'E', 'F', 'S']
        self.current_index = 0
        self.fullscreen = False
        
        # Configure the window
        self.root.title("Letter Cycle Game")
        self.root.geometry("800x600")
        self.root.configure(bg="black")
        
        # Create a large font for the letters
        self.large_font = font.Font(family="Arial", size=200, weight="bold")
        
        # Create the label to display the letter
        self.letter_label = tk.Label(
            self.root,
            text=self.letters[self.current_index],
        ...
        self.instructions = tk.Label(
            self.root,
            text="Press any key to cycle through letters (A,B,C,D,E,F,S) | F = Toggle Fullscreen | ESC/Q = Quit",
            font=("Arial", 12),
            fg="gray",
            bg="black"
        )
    ...
    def cycle_letter(self):
        ...
        self.letter_label.config(text=self.letters[self.current_index])
    ...
    def on_key_press(self, event):
        ...
        if key == 'f':
            ...
            # Any other key cycles through the letters
            self.cycle_letter()

def main():
    root = tk.Tk()
    game = LetterCycleGame(root)
    root.mainloop()
